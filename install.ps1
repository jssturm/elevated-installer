...
#   Or download + run: .\install.ps1

# Show welcome BEFORE any error handling that could suppress output
Write-Host ""
...
if (-not $hasTar) {
    Write-Host "  NOTE: 'tar' not found. Will try Expand-Archive for .zip files." -ForegroundColor Yellow
    Write-Host "  If the release is a .tar.gz, install tar or use WSL." -ForegroundColor Yellow
}
...
try {
    if ($DownloadPath -like "*.zip") {
        ...
        if ($LASTEXITCODE -eq 0) { $extractOk = $true }
            ...
            # Try stripping top-level directory
...
if (-not $extractOk) {
    ...
    if (-not $hasTar) {
        Write-Host "  Install 'tar' on Windows or use WSL for .tar.gz files." -ForegroundColor Yellow
    }
    Remove-Item -Recurse -Force $TempDir -ErrorAction SilentlyContinue
    exit 1
}
